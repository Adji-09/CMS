...
<html lang="id_ID.utf8">
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('adminlte/bower_components/bootstrap/dist/css/datepicker.min.css') }}">
        <!-- Font Awesome -->
        ...
        <link rel="stylesheet" href="{{ asset('adminlte/bower_components/datatables.net/css/select.dataTables.min.css') }}">
    ...
    <script src="{{ asset('adminlte/bower_components/bootstrap/dist/js/bootstrap-datepicker.min.js') }}"></script>
    <!-- Slimscroll -->
    ...
    <!-- PACE -->
    <!-- <script src="{{ asset('adminlte/bower_components/PACE/pace.js') }}"></script> -->
    ...
    <script src="{{ asset('adminlte/bower_components/datatables.net/js/dataTables.select.min.js') }}"></script>
    ...
    <script type="text/javascript">
        // SLIDE BANNER
        // ADD DATA
        $(document).on('click', '.slideBannerAdd-modal', function() {
            $('.modal-title').text('Add Data');
            $('#slideBannerAdd').modal('show');
        });

        // EDIT DATA
        $(document).on('click', '#editSlideBanner', function (event) {
            event.preventDefault();
            var id = $(this).data('id');
            console.log(id)
            $.get('slideBanner/edit/' + id, function (data) {
                $('.modal-title').text('Update Data');
                $('#slideBannerEdit').modal('show');
                $('#id').val(data.data.id);
        ...
        // DELETE DATA
        $(".deleteSlideBanner").click(function(e) {
            e.preventDefault();
            var id = $(this).data("id");
            ...
            if (confirm("Are you sure you want to Delete this data?")) {
                $.ajax({
                    url: "{{ url('/slideBanner/delete') }}/" + id,
                    type: 'DELETE',
                    data: {
                        "id": id,
                    },
                    success: function () {
                        alert('You have successfully delete your files');
                        window.location.href = "{{ url('/slideBanner') }}";
                    },
                    error: function() {
                        alert('Failed!');
                    }
        ...
        // PUBLIKASI
        // ADD DATA
        $(document).on('click', '.publikasiAdd-modal', function() {
            $('.modal-title').text('Add Data');
            $('#publikasiAdd').modal('show');
        });

        // EDIT DATA
        $(document).on('click', '#editPublikasi', function (event) {
            event.preventDefault();
            var id = $(this).data('id');
            console.log(id)
            $.get('publikasiCms/edit/' + id, function (data) {
                $('.modal-title').text('Update Data');
                $('#publikasiEdit').modal('show');
                $('#id').val(data.data.id);
                $('#judul_edit').val(data.data.judul);
                $('#des_singkat_edit').val(data.data.des_singkat);
                $('#image').val(data.data.image);
                $('#status_edit').val(data.data.status);
            })
        });
        ...
        // DELETE DATA
        $(".deletePublikasi").click(function(e) {
            e.preventDefault();
            var id = $(this).data("id");
            ...
            if (confirm("Are you sure you want to Delete this data?")) {
                $.ajax({
                    url: "{{ url('/publikasiCms/delete') }}/" + id,
                    type: 'DELETE',
                    data: {
                        "id": id,
                    },
                    success: function () {
                        alert('You have successfully delete your files');
                        window.location.href = "{{ url('/publikasiCms') }}";
                    },
                    error: function() {
                        alert('Failed!');
                    }
        ...
        // PERATURAN
        // ADD DATA
        $(document).on('click', '.peraturanAdd-modal', function() {
            $('.modal-title').text('Add Data');
            $('#peraturanAdd').modal('show');
        });

        // EDIT DATA
        $(document).on('click', '#editPeraturan', function (event) {
            event.preventDefault();
            var id = $(this).data('id');
            console.log(id)
            $.get('peraturan/edit/' + id, function (data) {
                $('.modal-title').text('Update Data');
                $('#peraturanEdit').modal('show');
                $('#id').val(data.data.id);
                $('#deskripsi_edit').val(data.data.deskripsi);
                $('#peraturan_edit').val(data.data.peraturan);
                $('#status_edit').val(data.data.status);
            })
        });

        // DELETE DATA
        $(".deletePeraturan").click(function(e) {
            e.preventDefault();
            var id = $(this).data("id");
            ...
            if (confirm("Are you sure you want to Delete this data?")) {
                $.ajax({
                    ...
                    success: function () {
                        alert('You have successfully delete your files');
                        window.location.href = "{{ url('/peraturan') }}";
                    },
                    error: function() {
                        alert('Failed!');
                    }
        ...
        // BASIS DATA
        // ADD DATA
        $(document).on('click', '.basisDataAdd-modal', function() {
            $('.modal-title').text('Add Data');
            $('#basisDataAdd').modal('show');
        });

        // EDIT DATA
        $(document).on('click', '#editBasisData', function (event) {
            event.preventDefault();
            var id = $(this).data('id');
            console.log(id)
            $.get('basisData/edit/' + id, function (data) {
                $('.modal-title').text('Update Data');
                $('#basisDataEdit').modal('show');
                $('#id').val(data.data.id);
                $('#nama_penulis_edit').val(data.data.nama_penulis);
                $('#tahun_edit').val(data.data.tahun);
                $('#jenis_hiu_edit').val(data.data.jenis_hiu);
                $('#deskripsi_edit').val(data.data.deskripsi);
                $('#image').val(data.data.image);
                $('#document_edit').val(data.data.document);
                $('#status_edit').val(data.data.status);
            })
        });

        // DELETE DATA
        $(".deleteBasisData").click(function(e) {
            e.preventDefault();
            var id = $(this).data("id");
            ...
            if (confirm("Are you sure you want to Delete this data?")) {
                $.ajax({
                    ...
                    success: function () {
                        alert('You have successfully delete your files');
                        window.location.href = "{{ url('/basisData') }}";
                    },
                    error: function() {
                        alert('Failed!');
                    }
        ...
        $(function () {
            CKEDITOR.replace('konten', {
                ...
                contentsCss: [
                    ...
                    'https://ckeditor.com/docs/ckeditor4/4.16.1/examples/https://ckeditor.com/docs/ckeditor4/4.16.1/examples/assets/css/pastefromword.css'
                ],
